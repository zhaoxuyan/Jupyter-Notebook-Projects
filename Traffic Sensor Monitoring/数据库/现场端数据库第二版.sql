...
create table sjzxb
(
   sjzxbh               varchar(16) not null comment '数据中心编号',
   sjzxmc               varchar(100) not null comment '数据中心名称',
   dz                   varchar(200) not null comment '地址',
   dk                   int not null comment '端口',
   scfs                 char(1) not null default '2' comment '上传方式',
   sc                   smallint not null default 10 comment '时长',
   sfqy                 char(1) not null default '1' comment '是否启用',
   primary key (sjzxbh)
)
engine = InnoDB
...
alter table sjzxb comment '数据中心表'$
